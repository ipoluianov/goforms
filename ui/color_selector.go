...
func NewColorSelector(parent Widget, col color.Color) *ColorSelector {
	var c ColorSelector
	c.resColor = col
	...
	btnCancel := pButtons.AddButtonOnGrid(2, 0, "Cancel", func(event *Event) {
		c.Reject()
	})
...
func (c *ColorSelector) addColorToPalette(col color.Color) {
	...
	btn := c.panelPalette.AddButtonOnGrid(c.panelPaletteCount%8, c.panelPaletteCount/8, text, func(event *Event) {
		c.SetColor(col)
	})
