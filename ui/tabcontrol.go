...
func (c *TabControl) updateHeaderButtons() {
	...
	for pageIndex, page := range c.pages {
		...
		btn := btnPanel.AddButtonOnGrid(0, 0, "TabName", func(event *Event) {
		...
		btnClose := btnPanel.AddButtonOnGrid(1, 0, "", func(event *Event) {
	...
	if c.showAddButton {
		c.btnAdd = c.header.AddButtonOnGrid(pageIndex+1, 0, "", func(event *Event) {
